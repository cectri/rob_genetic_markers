...
p + geom_point(data=df_R, aes(x=strain, y=robustness_n, color=strain),size=5)+
  ...
  theme(strip.text = element_text(face="bold"),
        text = element_text(size = 15),
        axis.text.x = element_text(size = 20, angle=90, vjust = 0.5,hjust = 1))
#ggsave("FR_overall.pdf", device= cairo_pdf, width = 15, height = 10, path = "~/Library/CloudStorage/OneDrive-Chalmers/PhD/Experimental_Data/Robusntess application on existing dataset/R_scripts/genetic_markers_robustness/plots/overall_new")
```
...
#ggsave("fitness_heatmap.pdf", device=cairo_pdf, width = 15, height = 10, path = "~/Library/CloudStorage/OneDrive-Chalmers/PhD/Experimental_Data/Robusntess application on existing dataset/R_scripts/genetic_markers_robustness/plots/overall_new")
```
### robusntess overall 
```{r}
results <- results %>%
...
df_mean_2 <- results %>%
  group_by(strain, p_space) %>% #calculating mean for each sample in the agar plate 
  mutate(mean=mean(mumax,  na.rm=TRUE)) %>%
  mutate(stdev=sd(mumax,  na.rm=TRUE)) %>%
...
ggsave("robustness_heatmap.pdf", device=cairo_pdf, width = 15, height = 10, path = "~/Library/CloudStorage/OneDrive-Chalmers/PhD/Experimental_Data/Robusntess application on existing dataset/R_scripts/genetic_markers_robustness/plots/overall_new")
```
