...
- plus two control strains
15) CEN.PK113-7D parental
16) Ethanol Red

The genes chosen above followed the analysis made with the R_mutants.Rmd script. 

The strains above were grown in three different perturbation spaces. The last one contained the following: 
16 perturbations containing different combinations of: four different malts + 12 hops + 6 aromas and 2 finnings
...
```{r}
summary <- read_excel("~/your_directory/media_composition_BPPP.xlsx", 
    sheet = "summary") 
head(summary)

strain <- read_excel("~/your_directory/media_composition_BPPP.xlsx", 
    sheet = "strain") 
head(strain)

QDR1_GBP2 <- read_excel("~/your_directory/part1_BPPP.xlsx", 
    sheet = "QDR1-GBP2_GV") %>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

MRP13_WWM1 <- read_excel("~/your_directory/part1_BPPP.xlsx", 
    sheet = "MRP13-WWM1_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 
...
HCM1_SMA2 <- read_excel("~/your_directory/part2_BPPP.xlsx", 
    sheet = "HCM1-SMA2_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

RPS14A_OCA4 <- read_excel("~/your_directory/part2_BPPP.xlsx", 
    sheet = "RPS14A-OCA4_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

MSH3_RPS14B <- read_excel("~/your_directory/part3_BPPP.xlsx", 
    sheet = "MSH3-RPS14B_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

BCH1_MET28 <- read_excel("~/your_directory/part3_BPPP.xlsx", 
    sheet = "BCH1-MET28_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

TIR3_HLJ1 <- read_excel("~/your_directory/part4_BPPP.xlsx", 
    sheet = "TIR3-HLJ1_GV")%>%
    mutate(timeh = `time`/60) %>%
    select(-`time`)%>%
    gather("well", "value", -timeh) 

CENPK_ETRED <- read_excel("~/your_directory/part4_BPPP.xlsx", 
...
# Convert 'replicate' to character type and 'media' into factors
df <- df %>%
...
#ggsave("part4_growtcurves.pdf", device=cairo_pdf, width = 15, height = 10, path = "~/your_directory/")
```
...
#ggsave("FR_BPS_2.pdf", device=cairo_pdf, width = 15, height = 10, path = "~/your_directory/")
```
